...
import { useShop } from '../../context/ShopContext'; // Import the useShop hook to access cart data
import { useNavigate } from 'react-router-dom';

function CartModal({ setCartModalVisible, cartModalVisible, cartData }) {
  const { cart, removeFromCart } = useShop(); // Access cart and removeFromCart function from context
  const navigate = useNavigate();

  const handleClose = () => {
    setCartModalVisible(false);  // Close the modal when the close button is clicked
  };
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
      <div className="bg-white w-full max-w-md p-6 rounded-lg shadow-lg relative">
        <button
          onClick={handleClose}  // Close the modal on button click
          ...
          <div>
            ...
            <div className="mt-6 flex justify-between items-center">
              <p className="text-lg font-semibold">
                Total: {cart.reduce((total, item) => total + item.price * item.quantity, 0)} Rs.
              </p>
              <button onClick={handleCheckout} className="bg-green-600 text-white py-2 px-6 rounded hover:bg-green-700">
                Check Out
              </button>
            </div>
          </div>
